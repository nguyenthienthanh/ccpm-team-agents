...
CCPM provides **native Bash script integrations** for external services, optimized for Claude Code:
...
# Test Slack
./.claude/scripts/slack-notify.sh '#dev-team' 'Test message'

# Test Confluence
echo "# Test Page\n\nContent here" > /tmp/test.md
./.claude/scripts/confluence-publish.sh 'DEV' 'Test Page' '/tmp/test.md'
```
...
# Method 2: Full URL
./.claude/scripts/figma-fetch.sh "https://www.figma.com/file/ABC123xyz456/Design-Name"
```
...
```bash
source .claude/.envrc
./.claude/scripts/slack-notify.sh '#dev-team' 'Hello from CCPM! 👋'
```
...
./.claude/scripts/confluence-publish.sh 'DEV' 'CCPM Test Page' '/tmp/test-doc.md'
```
...
3. Run: `bash .claude/scripts/figma-fetch.sh ABC123`
...
```bash
# Update JIRA
bash .claude/scripts/jira-update.sh ETHAN-1269 "Done" "Implementation completed"

# Notify Slack
bash .claude/scripts/slack-notify.sh '#dev-team' '✅ ETHAN-1269 completed! @reviewer-team please review'

# Publish docs to Confluence
bash .claude/scripts/confluence-publish.sh 'DEV' 'ETHAN-1269: Implementation' '.claude/logs/workflows/*/implementation.md'
```
...
**Usage:**
```bash
./.claude/scripts/jira-fetch.sh <ticket-key>
```
...
**Usage:**
```bash
./.claude/scripts/figma-fetch.sh <file-id-or-url>
```

**Examples:**
```bash
./.claude/scripts/figma-fetch.sh ABC123xyz456
./.claude/scripts/figma-fetch.sh "https://www.figma.com/file/ABC123xyz456/Design"
./.claude/scripts/figma-fetch.sh "https://www.figma.com/design/ABC123xyz456/Design"
```
...
**Usage:**
```bash
./.claude/scripts/slack-notify.sh <channel> <message> [thread_ts] [mentions]
```

**Examples:**
```bash
# Simple message
./.claude/scripts/slack-notify.sh '#dev-team' 'Deploy completed!'

# With mentions
./.claude/scripts/slack-notify.sh '#dev-team' 'PR ready for review' '' '@reviewer-team'

# Reply in thread
./.claude/scripts/slack-notify.sh '#dev-team' 'Updated' '1234567890.123456'

# Channel ID (not name)
./.claude/scripts/slack-notify.sh 'C01234567' 'Private channel message'
```
...
**Usage:**
```bash
./.claude/scripts/confluence-publish.sh <space-key> <title> <content-file> [parent-page-id]
```

**Examples:**
```bash
# Create new page
./.claude/scripts/confluence-publish.sh 'DEV' 'API Documentation' 'docs/api.md'

# Update existing page (same title = update)
./.claude/scripts/confluence-publish.sh 'DEV' 'API Documentation' 'docs/api-v2.md'

# Create as child page
./.claude/scripts/confluence-publish.sh 'DEV' 'Sub Page' 'docs/sub.md' '123456'
```
...
```bash
# .claude/scripts/slack-notify-deployment.sh
#!/bin/bash
ENVIRONMENT="$1"
VERSION="$2"
STATUS="$3"
...
bash .claude/scripts/slack-notify.sh '#deployments' "$MESSAGE"
```
...
- [ ] Edit `.claude/.envrc` with all tokens
- [ ] Run `source .claude/.envrc`
- [ ] Verify all scripts executable: `ls -l .claude/scripts/*.sh`

### JIRA
- [ ] JIRA API token generated
- [ ] `JIRA_URL`, `JIRA_EMAIL`, `JIRA_API_TOKEN` set
- [ ] Test: `./.claude/scripts/jira-fetch.sh <ticket>`

### Figma
- [ ] Figma access token generated
- [ ] `FIGMA_ACCESS_TOKEN` set
- [ ] Test: `./.claude/scripts/figma-fetch.sh <file-id>`
...
- [ ] Test: `./.claude/scripts/slack-notify.sh '#channel' 'test'`
...
- [ ] Test: `./.claude/scripts/confluence-publish.sh <space> <title> <file>`
...
- **JIRA Solution:** `.claude/docs/JIRA_WEBFETCH_SOLUTION.md`
- **JIRA Skill:** `.claude/skills/jira-fetch-webfetch.md`
- **JIRA Agent:** `.claude/agents/jira-operations.md`
