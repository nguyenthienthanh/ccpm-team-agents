...
This installs CCPM directly from the GitHub repository.
...
**Alternative (if added to official Claude Code marketplace):**
```bash
# If CCPM is listed in the official marketplace
...
│   └── project-contexts/         # Project configs
└── .claude-plugin/
...
**Note:** Your `.claude/` directory and project context are preserved. To completely remove CCPM:
```bash
...
**Error:**
```
bash: .claude/scripts/jira-fetch.sh: No such file or directory
```
...
**Error:**
```
Permission denied: .claude/scripts/jira-fetch.sh
```
...
3. **Set Up Integrations (if needed):**
   ...
   .claude/scripts/jira-fetch.sh YOUR-TICKET-123
   ```
...
5. **Explore Documentation:**
   ...
   - `.claude/docs/guides/` - Integration guides
   - `.claude/docs/phases/` - Phase-by-phase guides
...
Each phase has a detailed guide in `.claude/docs/phases/`.
