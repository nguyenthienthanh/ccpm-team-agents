...
Create `.claude/scripts/jira-fetch.sh`:
...
[Runs: bash .claude/scripts/jira-fetch.sh ETHAN-1269]
...
- [ ] `.claude/.envrc` configured with JIRA_URL, JIRA_EMAIL, JIRA_API_TOKEN
- [ ] Environment loaded: `source .claude/.envrc`
- [ ] Script exists: `.claude/scripts/jira-fetch.sh`
- [ ] Script executable: `chmod +x .claude/scripts/jira-fetch.sh`
- [ ] `jq` installed: `brew install jq` (macOS) or `apt install jq` (Linux)
- [ ] Bash permission in settings: `"Bash(bash **/.claude/scripts/**/*.sh:*)"`
- [ ] Test successful: `./.claude/scripts/jira-fetch.sh ETHAN-1269`
...
# 2. Create new token
# 3. Update .claude/.envrc
# 4. source .claude/.envrc
```
...
**Recommendation for Claude Desktop:** Use JIRA MCP (see `.claude/docs/guides/JIRA_INTEGRATION.md`)
...
- **Fetch Skill:** `.claude/skills/jira-fetch-webfetch.md`
- **MCP Integration:** `.claude/docs/guides/JIRA_INTEGRATION.md` (for Claude Desktop)
