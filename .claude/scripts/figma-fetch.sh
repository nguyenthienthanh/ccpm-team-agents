...
if [ -f ".claude/.envrc" ]; then
  ...
  echo "Please create .claude/.envrc with FIGMA_ACCESS_TOKEN"
  exit 1
fi
...
if ! command -v jq &> /dev/null; then
  ...
  # Save raw JSON
  mkdir -p .claude/logs/figma
  echo "$body" > ".claude/logs/figma/${FILE_ID}.json"
  echo ""
  echo "💾 Saved to: .claude/logs/figma/${FILE_ID}.json"
  exit 0
fi
...
echo "$body" > ".claude/logs/figma/${FILE_ID}.json"
echo ""
echo "💾 Full JSON saved to: .claude/logs/figma/${FILE_ID}.json"
...
if [ -n "$frames" ]; then
  ...
  echo "$images_response" > ".claude/logs/figma/${FILE_ID}-images.json"
  echo "✅ Image URLs saved to: .claude/logs/figma/${FILE_ID}-images.json"
else
