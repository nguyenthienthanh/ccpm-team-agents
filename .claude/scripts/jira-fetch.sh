#!/bin/bash
# JIRA Ticket Fetch Script for CCPM
...
if [ -f ".claude/.envrc" ]; then
  ...
  echo "Please create .claude/.envrc with JIRA credentials"
  exit 1
fi
...
if ! command -v jq &> /dev/null; then
  ...
  echo ""
  echo "💾 Saved to: .claude/logs/jira/${TICKET_KEY}.json"
  exit 0
fi
