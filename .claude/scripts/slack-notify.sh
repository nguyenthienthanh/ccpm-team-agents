...
if [ -z "$SLACK_BOT_TOKEN" ]; then
  ...
  echo "  5. Edit .claude/.envrc"
  echo "  6. Add: export SLACK_BOT_TOKEN=\"xoxb-your-token\""
  echo "  7. Run: source .claude/.envrc"
  exit 1
fi
...
# Save response
mkdir -p .claude/logs/slack
echo "$body" > ".claude/logs/slack/message-${message_ts}.json"
echo ""
echo "💾 Response saved to: .claude/logs/slack/message-${message_ts}.json"
