...
**Purpose:** Explain CCPM's dual-file loader CLAUDE.md architecture with Skills system

---

## 🚨 Architecture Decision: Dual-File Loader Architecture

**CCPM uses TWO files working together:**
1. **Project `.claude/CLAUDE.md`** - Lightweight loader (tells Claude to read plugin)
2. **Plugin `ccpm/CLAUDE.md`** - Complete CCPM system instructions
...
2. ~/.claude/plugins/.../CLAUDE.md (plugin) ← CCPM uses this
...
CCPM uses a **dual-file loader + project context** architecture:

1. **Project CLAUDE.md** (Loader) - Tells Claude to read plugin CLAUDE.md
2. **Plugin CLAUDE.md** (System) - ALL CCPM instructions (single source of truth)
...
Update when changing core CCPM behavior:
- Adding new agents
...
**How:**
1. Update `ccpm/CLAUDE.md` in repository
2. Bump version: `./scripts/sync-version.sh X.Y.Z`
3. Update CHANGELOG.md
4. Users update via: `/plugin update ccpm@ethan-ccpm`
...
/plugin install ccpm@ethan-ccpm  # Install if missing
```
...
- **Plugin CLAUDE.md:** `ccpm/CLAUDE.md`
- **Project Context Structure:** See `project:init` command
- **Plugin Installation:** `ccpm/docs/PLUGIN_INSTALLATION.md`
- **Project Initialization:** `ccpm/commands/project/init.md`
