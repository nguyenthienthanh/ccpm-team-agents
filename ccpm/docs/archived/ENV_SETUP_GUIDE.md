...
CCPM uses environment variables to configure integrations with external services like JIRA, Confluence, Slack, and Figma. This guide explains how to set up your environment variables properly.
...
export $(cat /path/to/ccpm/.envrc | grep -v "^#" | xargs)
```
...
**Testing:**
```bash
bash ccpm/scripts/slack-notify.sh "Test message from CCPM"
```
...
```bash
#!/bin/bash
# ccpm/scripts/check-integrations.sh
...
echo "  bash ccpm/scripts/jira-fetch.sh TICKET-123"
echo "  bash ccpm/scripts/slack-notify.sh 'Test message'"
echo "  bash ccpm/scripts/figma-fetch.sh FILE_KEY"
```

**Run it:**
```bash
bash ccpm/scripts/check-integrations.sh
```
...
**Create `.env`:**
```bash
cp ccpm/.envrc.template ccpm/.env
```
...
if [ -f "ccpm/.env" ]; then
  export $(cat ccpm/.env | grep -v "^#" | xargs)
fi
```
