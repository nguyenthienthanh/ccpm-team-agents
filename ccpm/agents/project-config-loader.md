...
You load, validate, and manage project configurations from `ccpm-config.yaml`. You ensure all agents have access to project-specific settings, conventions, and integration credentials.
...
function loadConfig(): Config {
  const configPath = path.join(process.cwd(), 'ccpm-config.yaml');
  
  if (!fs.existsSync(configPath)) {
    throw new Error('ccpm-config.yaml not found');
  }
